...
def find_track(track_name: str, limit: int = 10):
    return req.get(url + 'find_track/' + track_name + '?limit=' + str(limit)).json()

def find_file(file_name: str, limit: int = 10):
    return req.get(url + 'find_file/' + file_name + '?limit=' + str(limit)).json()
